...
class UTF8 {
 public:
  ...
  static constexpr const unsigned kMaxRune = 0x10FFFF;
  ...
  static int encode(OutputIt iter, unsigned int codepoint) {
    assert(!(codepoint > kMaxRune || (codepoint >= kSurrogateMin && kSurrogateMax)));
  ...
  static int decode(InputIt& iter, unsigned int& codepoint) {
    ...
    if (sz <= 3) {
      codepoint = (static_cast<unsigned>(b0 & kMask3) << 12) | (static_cast<unsigned>(b1 & kMaskx) << 6) |
    ...
    codepoint = (static_cast<unsigned>(b0 & kMask4) << 18) | (static_cast<unsigned>(b1 & kMaskx) << 12) |
                (static_cast<unsigned>(b2 & kMaskx) << 6) | static_cast<unsigned>(b3 & kMaskx);
